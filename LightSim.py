# Contains classes and functions to run simulation. 
# Reduced memory usage - only the final positions and velocities are saved.
# Used in testing to determine adequate time step sizes
...
def RK4(bodies, deltaTime, softeningRadius, startBodyIndex, endBodyIndex, positions, velocities):
    #4th order Ruge-Kutta method

    for i in range(startBodyIndex, endBodyIndex):
        x = Vector3Array([bodies[i].position, bodies[i].velocity])
        ...
        pos = x[0] + k1[0]/6 + k2[0]/3 + k3[0]/3 + k4[0]/6
        vel = x[1] + k1[1]/6 + k2[1]/3 + k3[1]/3 + k4[1]/6
